import { auto, forever, map } from 'async';

import { getCoreLns } from '../core-lightning/index.js';
import { getInfo } from '../core-lightning/methods.js';
import getTelegramBot from './get_telegram_bot.js';
import { postNodesOffline } from '../libs/core-telegram/index.js';
import runTelegramBot from './run_telegram_bot.js';
...
/** Connect nodes to Telegram

  {
    ...
    @returns via Promise
*/
const connectToTelegram = async args => {
  return await auto({
    // Check arguments
    validate: cbk => {
    ...
    getNodes: [
      ...
      async () => {
        ...
        return map(withName, async ({ coreln, node }) => {
          try {
            const wallet = await getInfo({ ln: coreln.ln });

            return { node, alias: wallet.alias, id: wallet.id };
          } catch (err) {
    ...
    getBot: [
      'validate',
      async ({}) => {
        return await getTelegramBot({ fs: args.fs, proxy: args.proxy });
      },
    ],
